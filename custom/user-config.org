...
**** ファイル用の設定
     ...
     (defun e:header-line-for-files-format ()
       ...
       (e:header-line-for-files-format-internal
        (f-long (or buffer-file-name magit-buffer-file-name))
        (if magit-buffer-refname (format "<%s>" magit-buffer-refname))))
   ...
   (use-package ranger
     ...
     (progn ;; ranger のヘッダーラインの表示を変更
       (defun e:advice:ranger--header-lhs:override (&rest args)
         ...
         (e:header-line-for-files-format-internal
          (abbreviate-file-name (or (r--fget ranger-current-file) ""))))
