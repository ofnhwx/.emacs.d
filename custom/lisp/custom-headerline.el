...
(progn ;; キャッシュ関連
  ...
  (defun chl--cache-exists (path)
    (and (local-variable-p 'chl--cache)
         (s-equals? (chl--get-cache-path) path)))
  (defun chl--make-cache (path)
    (let* ((proot (kllib:project-root path))
           (pname (kllib:project-name proot))
           (cache (list :path path :proot proot :pname pname)))
      (setq-local chl--cache cache))))
