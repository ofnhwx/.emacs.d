...
**** ファイル用の設定
     #+begin_src emacs-lisp
     (defvar e:header-line-for-files-cache (make-hash-table))
     (defun e:header-line-for-files-format ()
       "ファイル用のヘッダーラインのフォーマット"
       (let* ((path (f-long (or buffer-file-name magit-buffer-file-name "")))
              (refname (if magit-buffer-refname (format "<%s>" magit-buffer-refname) ""))
             ...
             (let* ((file  (f-filename path))
               ...
               (puthash key
                        (concat
                         ...
                         (propertize pname 'face 'font-lock-keyword-face)
                         dir
